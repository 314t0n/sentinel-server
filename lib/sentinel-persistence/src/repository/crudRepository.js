...
CrudRepository.prototype.update = function update(entity, response) {
    try {
        var query = this.databaseProvider.update(this.entityName, entity[this.entityName]);
        this.responseHandler.handleResponse(query, response);
    } catch (e) {
        response(e);
    }
...
CrudRepository.prototype.add = function add(entity, response) {
    try {
        var query = this.databaseProvider.save(this.entityName, entity[this.entityName]);
        this.responseHandler.handleResponse(query, response);
    } catch (e) {
        response(e);
    }
...
CrudRepository.prototype.remove = function remove(entity, response) {
    try {
        var query = this.databaseProvider.remove(this.entityName, {
            _id: entity.id
        });
        this.responseHandler.handleResponse(query, response);
    } catch (e) {
